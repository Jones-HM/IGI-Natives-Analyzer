push    ebp
mov     ebp, esp
sub     esp, 34Ch
push    ebx
push    esi
push    edi
call    sub_4C5810
mov     eax, [ebp+arg_0]
mov     esi, [ebp+arg_C]
mov     [ebp+var_48], eax
mov     [ebp+var_64], 40000000h
mov     eax, [eax+0Ch]
mov     [ebp+var_60], 1Eh
add     eax, 20h ; ' '
mov     [ebp+var_4C], eax
xor     eax, eax
cmp     esi, eax
mov     [ebp+var_5C], eax
mov     [ebp+var_58], eax
mov     [ebp+var_54], eax
mov     [ebp+var_50], eax
jle     short loc_4CBED6
mov     ecx, [ebp+arg_4]
mov     edx, [ebp+arg_10]
lea     edi, [ebp+var_34C]
lea     eax, [ebp+var_33C]
lea     ebx, [ecx+58h]
mov     ecx, [ebp+arg_8]
sub     ecx, edi
mov     dword ptr [ebp+var_8+4], esi
mov     dword ptr [ebp+var_10+4], ecx
jmp     short loc_4CBEA8
mov     ecx, dword ptr [ebp+var_10+4]
fld     qword ptr [edx]
lea     edi, [eax-10h]
add     edx, 8
add     eax, 18h
add     ebx, 70h ; 'p'
lea     esi, [edi+ecx]
mov     ecx, 6
rep movsd
fadd    qword ptr [eax-18h]
fstp    qword ptr [eax-18h]
mov     ecx, dword ptr [ebp+var_8+4]
mov     dword ptr [ebx-70h], 0
dec     ecx
mov     dword ptr [ebp+var_8+4], ecx
jnz     short loc_4CBEA5
mov     eax, [ebp+arg_8]
mov     ecx, 6
mov     esi, eax
lea     edi, [ebp+var_40]
rep movsd
mov     ecx, 6
mov     esi, eax
lea     edi, [ebp+var_28]
rep movsd
mov     esi, [ebp+arg_C]
test    esi, esi
jle     loc_4CBFC8
fld     [ebp+var_30]
lea     edx, [ebp+var_34C]
lea     ecx, [eax+10h]
sub     edx, eax
fld     [ebp+var_28]
fcomp   qword ptr [ecx-10h]
fnstsw  ax
test    ah, 1
jz      short loc_4CBF25
mov     eax, [ecx-10h]
mov     edi, [ecx-0Ch]
mov     dword ptr [ebp+var_28], eax
mov     dword ptr [ebp+var_28+4], edi
jmp     short loc_4CBF3E
fld     [ebp+var_40]
fcomp   qword ptr [ecx-10h]
fnstsw  ax
test    ah, 41h
jnz     short loc_4CBF3E
mov     eax, [ecx-10h]
mov     edi, [ecx-0Ch]
mov     dword ptr [ebp+var_40], eax
mov     dword ptr [ebp+var_40+4], edi
fld     [ebp+var_20]
fcomp   qword ptr [ecx-8]
fnstsw  ax
test    ah, 1
jz      short loc_4CBF59
mov     eax, [ecx-8]
mov     edi, [ecx-4]
mov     dword ptr [ebp+var_20], eax
mov     dword ptr [ebp+var_20+4], edi
jmp     short loc_4CBF72
fld     [ebp+var_38]
fcomp   qword ptr [ecx-8]
fnstsw  ax
test    ah, 41h
jnz     short loc_4CBF72
mov     eax, [ecx-8]
mov     edi, [ecx-4]
mov     dword ptr [ebp+var_38], eax
mov     dword ptr [ebp+var_38+4], edi
fld     [ebp+var_18]
fcomp   qword ptr [ecx]
fnstsw  ax
test    ah, 1
jz      short loc_4CBF8B
mov     eax, [ecx]
mov     edi, [ecx+4]
mov     dword ptr [ebp+var_18], eax
mov     dword ptr [ebp+var_18+4], edi
jmp     short loc_4CBF98
fcom    qword ptr [ecx]
fnstsw  ax
test    ah, 41h
jnz     short loc_4CBF98
fstp    st
fld     qword ptr [ecx]
fld     [ebp+var_18]
fcomp   qword ptr [edx+ecx]
fnstsw  ax
test    ah, 1
jz      short loc_4CBFAD
fld     qword ptr [edx+ecx]
fstp    [ebp+var_18]
jmp     short loc_4CBFBC
fcom    qword ptr [edx+ecx]
fnstsw  ax
test    ah, 41h
jnz     short loc_4CBFBC
fstp    st
fld     qword ptr [edx+ecx]
add     ecx, 18h
dec     esi
jnz     loc_4CBF0A
jmp     short loc_4CBFCB
fld     [ebp+var_30]
fld     [ebp+var_28]
fadd    ds:dbl_533928
mov     ecx, [ebp+arg_0]
mov     eax, [ecx+0DE0h]
mov     ebx, [ecx+0DE4h]
fstp    [ebp+var_28]
fld     [ebp+var_20]
fadd    ds:dbl_533928
mov     edx, [ecx+0DE8h]
fstp    [ebp+var_20]
fld     [ebp+var_18]
fadd    ds:dbl_534AF8
fstp    [ebp+var_18]
fld     [ebp+var_40]
fsub    ds:dbl_533928
fstp    [ebp+var_40]
fld     [ebp+var_38]
fsub    ds:dbl_533928
fstp    [ebp+var_38]
fsub    ds:dbl_534AF8
fld     [ebp+var_40]
fistp   qword ptr [ebp-10h]
mov     esi, dword ptr [ebp+var_10]
add     esi, eax
fld     [ebp+var_38]
fistp   qword ptr [ebp-8]
mov     ecx, dword ptr [ebp+var_8]
add     ecx, edx
mov     dword ptr [ebp+var_30+4], ecx
fistp   [ebp+var_10]
mov     edi, dword ptr [ebp+var_10]
mov     dword ptr [ebp+var_10+4], edx
add     edi, ebx
fld     [ebp+var_28]
fistp   [ebp+var_8]
mov     ecx, dword ptr [ebp+var_8]
add     ecx, eax
xor     ecx, esi
fld     [ebp+var_20]
fistp   [ebp+var_8]
mov     edx, dword ptr [ebp+var_8]
add     edx, ebx
mov     ebx, dword ptr [ebp+var_10+4]
xor     edx, edi
fld     [ebp+var_18]
fistp   [ebp+var_8]
mov     eax, dword ptr [ebp+var_8]
add     eax, ebx
mov     ebx, dword ptr [ebp+var_30+4]
xor     eax, ebx
or      eax, edx
or      eax, ecx
mov     dword ptr [ebp+var_8+4], eax
bsr     eax, dword ptr [ebp+var_8+4]
mov     dword ptr [ebp+var_8+4], eax
mov     edx, dword ptr [ebp+var_8+4]
mov     eax, 1Eh
sub     eax, edx
cmp     eax, 10h
jle     short loc_4CC095
mov     eax, 10h
mov     esi, [ebp+arg_8]
mov     edi, [ebp+arg_0]
push    eax
lea     ecx, [ebp+var_64]
push    esi
lea     edx, [ebp+var_64]
push    ecx
push    edx
push    edi
call    sub_4C9A30
mov     ebx, [ebp+arg_1C]
mov     dword ptr [ebp+var_10+4], eax
mov     eax, ebx
add     esp, 14h
and     eax, 7FFFFFFFh
jz      short loc_4CC0E3
mov     ecx, [ebp+arg_20]
mov     edx, [ebp+arg_18]
push    ecx
mov     ecx, [ebp+arg_C]
push    eax
mov     eax, [ebp+arg_14]
push    edx
push    eax
mov     eax, [ebp+arg_4]
lea     edx, [ebp+var_34C]
push    ecx
push    edx
push    esi
push    eax
push    edi
call    sub_4CDB20
add     esp, 24h
test    ebx, 80000000h
jz      loc_4CC28E
mov     eax, [ebp+arg_C]
test    eax, eax
jle     loc_4CC28E
mov     ecx, [ebp+arg_4]
mov     edi, [ebp+arg_10]
add     esi, 10h
mov     edx, eax
mov     [ebp+arg_8], edi
mov     [ebp+arg_10], esi
lea     ebx, [ecx+18h]
mov     [ebp+arg_C], edx
fld     qword ptr [edi]
fistp   [ebp+var_8]
mov     ecx, dword ptr [ebp+var_8]
test    ecx, ecx
jge     short loc_4CC11F
neg     ecx
fld     qword ptr [edi]
fcomp   ds:dbl_5333B0
fnstsw  ax
test    ah, 41h
jnz     short loc_4CC135
mov     eax, 1
jmp     short loc_4CC137
xor     eax, eax
mov     edx, dword ptr [ebp+var_10+4]
push    1
push    edx
push    ecx
push    eax
lea     eax, [esi-10h]
lea     ecx, [ebp+var_64]
push    eax
lea     edx, [ebp+var_D4]
push    ecx
push    edx
call    sub_4CB050
add     esp, 1Ch
test    eax, eax
jz      loc_4CC272
mov     eax, [ebx+40h]
xor     ecx, ecx
cmp     eax, ecx
jz      short loc_4CC1A2
fld     qword ptr [esi]
fsub    [ebp+var_BC]
fcom    ds:dbl_5333B0
fnstsw  ax
test    ah, 1
jz      short loc_4CC17E
fchs
fld     qword ptr [esi]
fsub    qword ptr [ebx]
fcom    ds:dbl_5333B0
fnstsw  ax
test    ah, 1
jz      short loc_4CC191
fchs
fxch    st(1)
fcomp   st(1)
fnstsw  ax
test    ah, 1
fstp    st
jz      loc_4CC272
mov     [ebp+var_164], ecx
mov     [ebp+var_DC], ecx
mov     [ebp+var_D8], ecx
lea     eax, [ebp+var_CC]
push    4
lea     ecx, [ebp+var_CC]
push    20000000h
lea     edx, [ebp+var_16C]
mov     [ebp+var_16C], eax
mov     eax, [ebp+arg_0]
push    ecx
push    edx
push    0Bh
push    eax
mov     [ebp+var_160], 1
mov     [ebp+var_E0], 0FFh
mov     [ebp+var_168], 1
call    sub_4CE090
mov     al, [ebp+var_160]
add     esp, 18h
test    al, al
jz      short loc_4CC272
fld     [ebp+var_BC]
fsub    qword ptr [esi]
fcom    ds:dbl_5333B0
fnstsw  ax
test    ah, 1
jz      short loc_4CC21C
fchs
mov     edx, [ebp+var_84]
lea     edi, [ebx-10h]
mov     ecx, 6
lea     esi, [ebp+var_CC]
rep movsd
fstp    qword ptr [ebx+48h]
mov     ecx, [ebp+var_88]
lea     eax, [ebx+20h]
mov     [ebx+34h], ecx
mov     ecx, [ebp+var_9C]
mov     esi, [ebp+arg_10]
mov     edi, [ebp+arg_8]
mov     [ebx+38h], edx
mov     edx, [ebp+var_98]
mov     [eax], ecx
mov     ecx, [ebp+var_94]
mov     dword ptr [ebx+40h], 1
mov     dword ptr [ebx-18h], 0
mov     [eax+4], edx
mov     [eax+8], ecx
mov     eax, [ebp+arg_C]
add     ebx, 70h ; 'p'
add     edi, 8
add     esi, 18h
dec     eax
mov     [ebp+arg_8], edi
mov     [ebp+arg_10], esi
mov     [ebp+arg_C], eax
jnz     loc_4CC111
pop     edi
pop     esi
pop     ebx
mov     esp, ebp
pop     ebp
retn
