char __cdecl sub_443A60(int a1)
{
  char result; // al
  int v2; // edi
  int v3; // eax
  ...
  if ( !result )
  {
    *(_BYTE *)(a1 + 584) = 1;
    ...
    SymbolRegisterBool8((char)Buffer, a1 + 568);
    ...
    GameDataSymbolLoad(Buffer, "%s.isHacked", v4);
    SymbolRegisterBool8((char)Buffer, a1 + 577);
    GameDataSymbolLoad(Buffer, "%s.isHackedThisTick", v4);
